...
public class JdbcUserDaoTests extends BaseDaoTests {
    protected static final User USER_1 = new User(1, "user1", "user1", "user1", "user1", "ROLE_USER");
    protected static final User USER_2 = new User(2, "user2", "user2", "user2", "user2", "ROLE_USER");
    private static final User USER_3 = new User(3, "user3", "user3","user3", "user3", "ROLE_USER");
    ...
    public void create_user_with_null_firstName() {
        sut.create(null, USER_3.getLastName(),USER_3.getPassword(), "ROLE_USER");
    }

    @Test(expected = DataIntegrityViolationException.class)
    public void create_user_with_null_lastName() {
        sut.create(USER_3.getFirstName(), null, USER_3.getPassword(), "ROLE_USER");
    }


    @Test(expected = IllegalArgumentException.class)
    public void create_user_with_null_password() {
        sut.create(USER_3.getFirstName(),USER_3.getLastName(), null, "ROLE_USER");
    }

    @Test
    public void create_user_creates_a_user() {
        User newUser = new User(-1, "new", "new", "nnew-1", "user", "ROLE_USER");

        User nonNullUser = sut.create(newUser.getFirstName(), newUser.getLastName(), newUser.getPassword(), "ROLE_USER");
        boolean userWasCreated = nonNullUser.getId() == newUser.getId() &&
                Objects.equals(nonNullUser.getFirstName(), newUser.getFirstName()) &&
                Objects.equals(nonNullUser.getLastName(), newUser.getLastName());
